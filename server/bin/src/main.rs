...
async fn main() {
  let config_path = if cfg!(debug_assertions) {
   ...
   // server_initialize::initialize_config_with_env( "server/resources/application-test.yaml", Some("APP")).await;
    // 使用多实例环境变量优先的配置加载方式
    // 支持单个配置项和多实例配置的环境变量覆盖
    server_initialize::initialize_config_with_multi_instance_env(config_path, Some("APP")).await;
