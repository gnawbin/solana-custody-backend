        ...
        openapi.components = Some(
           ComponentsBuilder::new()
                .security_scheme(
                        ...
                        "bearer_auth",
                        SecurityScheme::Http(HttpBuilder::new().scheme(HttpAuthScheme::Bearer).into())).build(),
        );
    }
}

// Optional, in case the warning persists
#[derive(OpenApi,ToSchema)]
#[openapi(
    components(schemas(ApiDoc)),
    security(
        ("bearer_auth" = ["Bearer"])
    ),
...
pub async fn initialize_admin_router() -> Router {
    ...
    app = app.merge(SwaggerUi::new("/swagger-ui").url("/api-docs/openapi.json", ApiDoc::openapi())).fallback(handler_404);
